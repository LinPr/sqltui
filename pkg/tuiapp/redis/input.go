...
func addCommandHistory(cmd string) {
	CommandHistories = append(CommandHistories, cmd)
	if len(CommandHistories) > 100 {
		CommandHistories = CommandHistories[len(CommandHistories)-100:]
	}
}
...
func RenderInputFiedl() *tview.InputField {
	...
	inputField := tview.NewInputField().
		SetLabel("Query: ").
		SetPlaceholder("Enter redis query here...").
	...
	inputField.SetAutocompleteFunc(func(currentText string) (entries []string) {
		...
		var tipCmd []RedisHelp
		for _, cmdHelp := range commandHelps {
			if strings.HasPrefix(strings.ToLower(cmdHelp.Command), strings.ToLower(strings.Trim(currentText, " "))) {
				entries = append(entries, cmdHelp.Command)
				tipCmd = append(tipCmd, cmdHelp)
			}
		}
		if len(entries) >= 1 {
			PrintfErrTextView("[dark]\n\t%s %s", tipCmd[0].Command, tipCmd[0].Args)
		}

		for _, cmdHistory := range CommandHistories {
			if strings.HasPrefix(strings.ToLower(cmdHistory), strings.ToLower(currentText)) {
				entries = append(entries, cmdHistory)
			}
		}
